...
router.get('/all', function (req, res) {
  ...
  router.get('/getCustomerItems/:customer_id', function(req, res) {
    ...
    const query = 'SELECT item.Item_id AS Item_ID, item.Name AS Item_Name, Total_price FROM orders INNER JOIN item ON orders.Item_id = item.Item_id INNER JOIN customer ON orders.Customer_id = customer.Id_no WHERE customer.Id_no = ?';
